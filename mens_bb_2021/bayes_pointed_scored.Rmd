...
This exercise was performed and it was found to be very true.  This model's prediction power is optimized near a weighting of ~35%, where prediction accuracy settles around 80-83%.  This is a tremendous improvement of the previous model, which saw success rates ~67%.   
...
ggplot(results,aes(x=share,y=avg)) + geom_line() + ylab('Rate of Correct Predictions') + xlab('Weight Applied to Scoring Hyperparameters') + scale_y_continuous(label=scales::percent) + scale_x_continuous(label=scales::percent) + ggtitle('Results of Weighting Experiment: Accuracy as a Function of Weighting')
```

```{r GEN OPTIMAL MODEL, echo = FALSE}
w35 <- makePreds(1000,10000,.35)
w35 <- seedCleaning(w35)
```

Predictions rates across our different seed spreads are, as expected, also greatly improved.  We have an 80% accuracy for any games involving seed spreads between -15 and -1.  For games with a spread of 0, we have an accuracy of 70%, which seems very good for games that most would consider a coin-toss.  For upsets (seed spreads between 1 and 8), we have success rates above 50%.  

```{r summary by seed spread 2, echo = FALSE, warnings = FALSE, message = FALSE}
...
Although more research is needed, this drastic improvement might be explained by a few interesting anecdotals.  One great example is the Davidson vs Wisconsin matchup from 2008 that we described earlier.  Although our old model gave Wisconsin a slight advantage, our new model gives Davidson a far greater probability of winning.  Our new model would have given Davidson a 90% chance of victory compared to the previous estimate of 48%.  

```{r weight stuff toward offense, include = FALSE}
model35 <- modelTest('Davidson','Wisconsin','2008',.35)
prob1 <- sum(model35$diff >= 0)/500000
...
view2 <- ggplot(model35,aes(x=diff)) + geom_histogram(binwidth=1) + coord_cartesian(xlim=c(-10,10)) + xlab('Difference in Points') + xlab('Total Point Spread in Favor of Davidson') + ylab('Total Volume of 50k Simulations') + geom_vline(xintercept=0,linetype='dashed',colour='red') + ggtitle('Model with 35% Weighted Added to Offensive Data')
